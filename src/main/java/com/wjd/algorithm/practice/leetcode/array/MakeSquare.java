 ...
 * @since 2022/6/1
 */
public class MakeSquare implements Solution<Boolean> {
    ...
    public boolean makesquare(int[] matchsticks) {
        ...
        return backtrace(matchsticks, visited, side, 0);
    }
     ...
     */
    private boolean backtrace(int[] matchsticks, boolean[] visited, int side, int k) {
        // 寻找还未用到的火柴
        int next = -1;
        ...
        for (int i = next; i < visited.length; i++) {
            ...
            visited[i] = true;
            if (backtrace(matchsticks, visited, side, k + matchsticks[i])) {
                return true;
            }
            visited[i] = false;
        }
