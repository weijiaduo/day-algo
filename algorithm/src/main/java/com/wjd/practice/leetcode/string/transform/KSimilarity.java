...
public class KSimilarity {
    ...
    private int kSimilarity(String s1, String s2) {
        ...
        backtrack(s1.toCharArray(), s2.toCharArray(), 0, 0);
        return ans;
    }
     ...
     */
    private void backtrack(char[] s1, char[] s2, int index, int cnt) {
        // 快速剪枝，交换次数比最小值大了
        if (cnt >= ans) {
            return;
        }
